    ...
    public function getToken($request, $response, $args){
       include("error.php");
        ...
        if (strpos($corpName, "PLT-Admin") !== false) {
            $_name2 =str_replace("PLT-Admin", "", $corpName);;
        } else {
        ...
        $out["status"] = "OK";
        $out["url"] = "https://pinlet.app/diners/?token=".$stringConcatenado;
    ...
    public function addInvitation($request, $response, $args){
       ...
       $response = $response->withHeader('Content-Type', 'application/json');
        ...
        if (strtotime($start_date) < strtotime($current_date)) {
            // La fecha de inicio es anterior al día actual
            $out["error"] = "ERROR";
            $out["codigo"] = "201";
            $out["mensaje"] = "La fecha de inicio debe ser durante el día actual o posterior.";
            $out["causa"] = $error_202_causa;
            $errors[] = "Start date must be during the current day or later.";
        }
        ...
        if (!preg_match('/^[a-zA-Z]+ [a-zA-Z]+/', $visitor)) {
          $out["error"] = "ERROR";
            $out["codigo"] = "201";
        ...
        if (empty($errors)) {
          ...
          $fechaConvertida = date('YmdHis', strtotime($start_date));
          $fechaConvertida2 = date('YmdHis', strtotime($end_date));
          $codigo=$resident."TTI".$id_invitacion."ID".$fechaConvertida."FI".$fechaConvertida2."FT";
    ...
    public function getInvitation($request, $response, $args){
       ...
       $response = $response->withHeader('Content-Type', 'application/json');
        ...
        try {
            ...
            if (strpos($corpName, "PLT-Admin") !== false) {
                $sql = "SELECT * FROM pl_invitacion_ocasional i WHERE id_invitacion_normal = :id_invitation";
            }
            $statement = $db->prepare($sql);
            ...
            while($item = $statement->fetch()){
                ...
                $fechaConvertida = date('YmdHis', strtotime($item->fecha_inicio));
                $fechaConvertida2 = date('YmdHis', strtotime($item->fecha_termino));
                $codigo=$item->socio."TTI".$id_invitation."ID".$fechaConvertida."FI".$fechaConvertida2."FT";
                $codigoF = base64_encode($codigo);
                $out["qr"] = $codigoF;
    ...
    public function getGroupInvitation($request, $response, $args){
       ...
       $response = $response->withHeader('Content-Type', 'application/json');
        ...
        try {
            ...
            if (strpos($corpName, "PLT-Admin") !== false) {
                $sql = "SELECT * FROM pl_invitacion_ocasional i WHERE id_invitacion_diners = :id_diners";
            }
            $statement = $db->prepare($sql);
            $statement->bindValue(':id_diners',  $id_diners, \PDO::PARAM_STR);
            $result = $statement->execute();
            ...
            while($item = $statement->fetch()){
                ...
                $fechaConvertida = date('YmdHis', strtotime($item->fecha_inicio));
                $fechaConvertida2 = date('YmdHis', strtotime($item->fecha_termino));
                $codigo=$item->socio."TTI".$item->id_invitacion_normal."ID".$fechaConvertida."FI".$fechaConvertida2."FT";
                $codigoF = base64_encode($codigo);
                $out1["qr"] = $codigoF;
                $out[] = $out1
                ;
        ...
        try {
            ...
            while($item = $statement->fetch()){
              if ($item->fecha_ingreso === null && $item->fecha_salida === null && $item->fecha_termino > date('Y-m-d H:i:s')) {
                ...
                if (strpos($corpName, "PLT-Admin") !== false) {
                  $sql = "DELETE FROM pl_invitacion_ocasional WHERE id_invitacion_normal=:id_invitation ";
                }
                $statement = $db->prepare($sql);
                $statement->bindValue(':id_invitation',  $id_invitation, \PDO::PARAM_STR);
                $result = $statement->execute();
