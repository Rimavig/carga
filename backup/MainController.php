    ...
    public function addInvitation($request, $response, $args){
       ...
       $response = $response->withHeader('Content-Type', 'application/json');
        ...
        if (empty($errors)) {
          ...
          $codigo=$resident."TT2".$id_invitacion."ID".$fechaConvertida."FI".$fechaConvertida2."FT";
       ...
       $response = $response->withHeader('Content-Type', 'application/json');
        ...
        try {
            ...
            while($item = $statement->fetch()){
                ...
                $codigo=$item->socio."TT2".$id_invitation."ID".$fechaConvertida."FI".$fechaConvertida2."FT";
                $codigoF = base64_encode($codigo);
                $out["qr"] = $codigoF;
       ...
       $response = $response->withHeader('Content-Type', 'application/json');
        ...
        try {
            ...
            while($item = $statement->fetch()){
                ...
                $fechaConvertida = date('YmdHis', strtotime($item->fecha_inicio));
                $fechaConvertida2 = date('YmdHis', strtotime($item->fecha_termino));
                $codigo=$item->socio."TT2".$item->id_invitacion_normal."ID".$fechaConvertida."FI".$fechaConvertida2."FT";
                $codigoF = base64_encode($codigo);
                $out1["qr"] = $codigoF;
                $out[] = $out1
                ;
